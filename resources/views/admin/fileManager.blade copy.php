                        ...
                        <div class="fm-menu">
                            <div class="list-group list-group-flush">
                                @foreach ($directoryContents as $item)
                                    {{-- <a href="{{ route('singkuasa.fileManager', $item) }}" class="list-group-item py-1">
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        {{-- @php
                            ...
                            $extensionSizes = [
                                'php' => convertToBytes($phpTotalSize),
                                'js' => convertToBytes($jsTotalSize),
                                'html' => convertToBytes($htmlTotalSize),
                                'css' => convertToBytes($cssTotalSize),
                                'jpg' => convertToBytes($imgTotalSize),
                                'gif' => convertToBytes($imgTotalSize),
                                'sql' => convertToBytes($sqlTotalSize),
                                // Tambahkan ekstensi lainnya dengan ukuran masing-masing
                            ];
                            
                            foreach ($extensionsCount as $extension => $count) {
                                if (array_key_exists($extension, $extensionSizes)) {
                                    $totalSize += $extensionSizes[$extension];
                                }
                            }
                            
                            // Tambahkan ukuran unformat files ke totalSize
                            $totalSize += convertToBytes($unformatFilesSize);
                        ...
                        <div class="progress mt-3" style="height: 7px;">
                            @foreach ($extensionsCount as $extension => $count)
                                @if ($extension === 'php' || $extension === 'js' || $extension === 'html' || $extension === 'css' || $extension === 'sql' || in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'avi', 'mov'], true))
                                    @php
                                        $progressColor = '';
                                        $extensionSize = 0;
                                        switch ($extension) {
                                            case 'php':
                                                ...
                                                $extensionSize = $phpTotalSize;
                                                break;
                                            case 'js':
                                                $progressColor = 'bg-warning';
                                                $extensionSize = $jsTotalSize;
                                                break;
                                            case 'html':
                                                $progressColor = 'bg-success';
                                                $extensionSize = $htmlTotalSize;
                                                break;
                                            case 'css':
                                                $progressColor = 'bg-primary';
                                                $extensionSize = $cssTotalSize;
                                                break;
                                            case 'jpg':
                                            case 'jpeg':
                                            case 'png':
                                            case 'gif':
                                                ...
                                                $extensionSize = $imgTotalSize;
                                                break;
                                            case 'mp4':
                                            case 'avi':
                                            case 'mov':
                                                $progressColor = 'bg-black';
                                                $extensionSize = $videoTotalSize;
                                                break;
                                            case 'sql':
                                                $progressColor = 'bg-light';
                                                $extensionSize = $sqlTotalSize;
                                                break;
                                            default:
                                                $unformatFilesSize = $unformatFilesSize;
                                                break;
                                        }
                                    @endphp
                                    <div class="progress-bar {{ $progressColor }}" role="progressbar"
                                        style="width: {{ ($extensionSize / $totalSize) * 100 }}%"
                                        aria-valuenow="{{ ($extensionSize / $totalSize) * 100 }}" aria-valuemin="0"
                                        aria-valuemax="100"></div>
                                @endif
                            @endforeach
                            <div class="progress-bar bg-light" role="progressbar"
                                style="width: {{ ($unformatFilesSize / $totalSize) * 100 }}%"
                                aria-valuenow="{{ ($unformatFilesSize / $totalSize) * 100 }}" aria-valuemin="0"
                                aria-valuemax="100"></div>
                        </div> --}}
                        <div class="progress mt-3" style="height: 7px;">
                            @foreach ($extensionsCount as $extension => $count)
                                @if (
                                    $extension === 'php' ||
                                        ...
                                        $extension === 'css' ||
                                        $extension === 'sql' ||
                                        in_array($extension, ['jpg', 'jpeg', 'png', 'gif', 'mp4', 'avi', 'mov'], true))
                                    @php
                                        $progressColor = '';
                                        switch ($extension) {
                                            case 'php':
                                    ...
                                    <div class="progress-bar {{ $progressColor }}" role="progressbar"
                                        ...
                                        aria-valuenow="{{ ($count / $totalFiles) * 100 }}" aria-valuemin="0"
                                        aria-valuemax="100"></div>
                                @endif
                            @endforeach
                            <div class="progress-bar bg-light" role="progressbar"
                                style="width: {{ ($unformatFilesCount / $totalFiles) * 100 }}%"
                                aria-valuenow="{{ ($unformatFilesCount / $totalFiles) * 100 }}" aria-valuemin="0"
                                aria-valuemax="100"></div>
                        </div>
                            ...
                            @if (
                                ...
                                <div class="d-flex align-items-center mt-2">
                                    ...
                                    <div class="flex-grow-1 ms-2">
                                        <h6 class="mb-0">{{ strtoupper($extension) }}</h6>
                                        <p class="mb-0 text-secondary">{{ $count }} Files</p>
                                    </div>
