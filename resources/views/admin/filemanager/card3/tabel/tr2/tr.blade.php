<tr>
    <td>
        <div class="d-flex align-items-center">
            ...
            @elseif ($item['extension'] == 'mp4')
                @include('admin.filemanager.card3.tabel.tr2.td.divVideo')
            @elseif (
                $item['extension'] == 'jpg' ||
                    $item['extension'] == 'jpeg' ||
                    $item['extension'] == 'png' ||
    ...
    <td>
        ...
        @elseif(strpos($item['name'], '.mp4') !== false)
            <a class="me-2 font-24 renameFiFo" style="text-decoration: none; color: black;" href="#"
                title="Rename {{ $item['name'] }}" data-toggle="modal" data-target="#editModal"
                data-name="{{ $folderPathNow }}" data-original-name="{{ $item['name'] }}">
                <i class='bx bx-edit-alt'></i>
            </a>
            <a class="me-2 font-24 playVideo" target="_blank" target-play="{{ $folderPathNow . '/' . $item['name'] }}"
                style="text-decoration: none; color: black;" href="#" title="Play {{ $item['name'] }}">
